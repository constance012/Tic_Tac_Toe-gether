...
public sealed class ObjectPool<TObject> where TObject : MonoBehaviour, IPoolable
{
	...
	{
		return AddNewGameObjectToPool(prefab);
	}
	...
	private TObject TrySpawnObject(Func<TObject, bool> predicate)
	{
		...
		if (_pool.Count == 0)
		{
			Debug.LogWarning($"The pool of {typeof(TObject)} is empty, instantiating a new object.");
			newObject = AddNewGameObjectToPool(_prefab);
			newObject.Allocate();
		}
		else
		{
			...
			{
				Debug.LogWarning("Not enough object to spawn, try deallocating some already allocated ones. Instantiating a new object.");
				newObject = AddNewGameObjectToPool(_prefab);
				newObject.Allocate();
			}
		}
	...
	private TObject AddNewGameObjectToPool(GameObject prefab)
	{
